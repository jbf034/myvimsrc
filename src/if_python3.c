...
#define PyString_AsBytes(obj) PyUnicode_AsEncodedString(obj, (char *)p_enc, "surrogateescape");
...
DoPy3Command(exarg_T *eap, const char *cmd)
{
    ...
    cmdstr = PyUnicode_Decode(cmd, strlen(cmd), (char *)p_enc, "surrogateescape");
    cmdbytes = PyUnicode_AsEncodedString(cmdstr, "utf-8", "surrogateescape");
    Py_XDECREF(cmdstr);
...
LineToString(const char *str)
{
    ...
    *p = '\0';
    ...
    result = PyUnicode_Decode(tmp, len, (char *)p_enc, "surrogateescape");
